...
#ifdef EEEPC_BUILD1
static void fuel_eff_formula(int data, char *buf);
#endif 
...
static SENSOR sensors[] =
{
   ...
   { intake_pressure_formula,       "Intake Pressure:",         "", "0B",      1,    1 },
   { air_flow_rate_formula,         "Mass Air Flow:",           "", "10",      1,    2 },
   { fuel_system1_status_formula,   "Fuel System 1:",           "", "03",      1,    2 },
   { fuel_system2_status_formula,   "Fuel System 2:",           "", "03",      1,    2 },
   // Page 2
   { short_term_fuel_trim_formula,  "S.T. Fuel Trim 1:",        "", "06",      1,    2 },
   { long_term_fuel_trim_formula,   "L.T. Fuel Trim 1:",        "", "07",      1,    2 },
   { short_term_fuel_trim_formula,  "S.T. Fuel Trim 2:",        "", "08",      1,    2 },
   { long_term_fuel_trim_formula,   "L.T. Fuel Trim 2:",        "", "09",      1,    2 },
   { intake_air_temp_formula,       "Intake Air Temp.:",        "", "0F",      1,    1 },
   { coolant_temp_formula,          "Coolant Temp.:",           "", "05",      1,    1 },
   { fuel_pressure_formula,         "Fuel Pressure:",           "", "0A",      1,    1 },
   { secondary_air_status_formula,  "Second. air status:",      "", "12",      1,    1 },
   { pto_status_formula,            "Aux. snput status:",       "", "1E",      1,    1 },
   // Page 3
   { o2_sensor_formula,             "O2 #1, Bank 1:",           "", "14",      1,    2 },
   { o2_sensor_formula,             "O2 #2, Bank 1:",           "", "15",      1,    2 },
   { o2_sensor_formula,             "O2 #3, Bank 1:",           "", "16",      1,    2 },
   { o2_sensor_formula,             "O2 #4, Bank 1:",           "", "17",      1,    2 },
   { o2_sensor_formula,             "O2 #1, Bank 2:",           "", "18",      1,    2 },
   { o2_sensor_formula,             "O2 #2, Bank 2:",           "", "19",      1,    2 },
   { o2_sensor_formula,             "O2 #3, Bank 2:",           "", "1A",      1,    2 },
   { o2_sensor_formula,             "O2 #4, Bank 2:",           "", "1B",      1,    2 },
   { obd_requirements_formula,      "OBD standard:",            "", "1C",      1,    1 },
   // Page 4
   { o2_sensor_wrv_formula,         "O2 #1, Bank 1(WR):",       "", "24",      1,    4 },    // o2 sensors (wide range), voltage
   { o2_sensor_wrv_formula,         "O2 #2, Bank 1(WR):",       "", "25",      1,    4 },
   { o2_sensor_wrv_formula,         "O2 #3, Bank 1(WR):",       "", "26",      1,    4 },
   { o2_sensor_wrv_formula,         "O2 #4, Bank 1(WR):",       "", "27",      1,    4 },
   { o2_sensor_wrv_formula,         "O2 #1, Bank 2(WR):",       "", "28",      1,    4 },
   { o2_sensor_wrv_formula,         "O2 #2, Bank 2(WR):",       "", "29",      1,    4 },
   { o2_sensor_wrv_formula,         "O2 #3, Bank 2(WR):",       "", "2A",      1,    4 },
   { o2_sensor_wrv_formula,         "O2 #4, Bank 2(WR):",       "", "2B",      1,    4 },
   { engine_run_time_formula,       "Engine Uptime:",           "", "1F",      1,    2 },
   // Page 5
   { frp_relative_formula,          "Fuel Pres. (vacuum):",     "", "22",      1,    2 },    // fuel rail pressure relative to manifold vacuum
   { frp_widerange_formula,         "Fuel Pres. (gauge):",      "", "23",      1,    2 },    // fuel rail pressure (gauge), wide range
   { commanded_egr_formula,         "Commanded EGR:",           "", "2C",      1,    1 },
   { egr_error_formula,             "EGR Error:",               "", "2D",      1,    1 },
   { evap_pct_formula,              "Commanded Evap Purge:",    "", "2E",      1,    1 },
   { fuel_level_formula,            "Fuel Level Input:",        "", "2F",      1,    1 },
   { warm_ups_formula,              "Warm-ups since reset:",    "", "30",      1,    1 },
   { clr_distance_formula,          "Distance since reset:",    "", "31",      1,    2 },
   { evap_vp_formula,               "Evap System Vapor Pres.:", "", "32",      1,    2 },
   // Page 6
   { o2_sensor_wrc_formula,         "O2 #1, Bank 1(WR):",       "", "34",      1,    4 },   // o2 sensors (wide range), current
   { o2_sensor_wrc_formula,         "O2 #2, Bank 1(WR):",       "", "35",      1,    4 },
   { o2_sensor_wrc_formula,         "O2 #3, Bank 1(WR):",       "", "36",      1,    4 },
   { o2_sensor_wrc_formula,         "O2 #4, Bank 1(WR):",       "", "37",      1,    4 },
   { o2_sensor_wrc_formula,         "O2 #1, Bank 2(WR):",       "", "38",      1,    4 },
   { o2_sensor_wrc_formula,         "O2 #2, Bank 2(WR):",       "", "39",      1,    4 },
   { o2_sensor_wrc_formula,         "O2 #3, Bank 2(WR):",       "", "3A",      1,    4 },
   { o2_sensor_wrc_formula,         "O2 #4, Bank 2(WR):",       "", "3B",      1,    4 },
   { mil_distance_formula,          "Distance since MIL:",      "", "21",      1,    2 },
   // Page 7
   { baro_pressure_formula,         "Abs. Baro Pressure:",      "", "33",      1,    1 },
   { cat_temp_formula,              "CAT Temp., B1S1:",         "", "3C",      1,    2 },
   { cat_temp_formula,              "CAT Temp., B2S1:",         "", "3D",      1,    2 },
   { cat_temp_formula,              "CAT Temp., B1S2:",         "", "3E",      1,    2 },
   { cat_temp_formula,              "CAT Temp., B2S2:",         "", "3F",      1,    2 },
   { ecu_voltage_formula,           "ECU voltage:",             "", "42",      1,    2 },
   { abs_load_formula,              "Abs. Engine Load:",        "", "43",      1,    2 },
   { eq_ratio_formula,              "Commanded Eq. Ratio:",     "", "44",      1,    2 },
   { amb_air_temp_formula,          "Ambient Air Temp,:",       "", "46",      1,    1 },  // same scaling as $0F
   // Page 8
   { relative_tp_formula,           "Rel. Throttle Position:",  "", "45",      1,    1 },
   { abs_tp_formula,                "Abs. Throttle Position B:","", "47",      1,    1 },
   { abs_tp_formula,                "Abs. Throttle Position C:","", "48",      1,    1 },
   { abs_tp_formula,                "Accel. Pedal Position D:", "", "49",      1,    1 },
   { abs_tp_formula,                "Accel. Pedal Position E:", "", "4A",      1,    1 },
   { abs_tp_formula,                "Accel. Pedal Position F:", "", "4B",      1,    1 },
   { tac_pct_formula,               "Comm. Throttle Actuator:", "", "4C",      1,    1 }, // commanded TAC
   { mil_time_formula,              "Time Run with MIL on:",    "", "4D",      1,    2 }, // minutes run by the engine while MIL activated
   ...
   { fuel_system1_status_formula,   "Fuel System 1:",           "", "03",      1,    2 },
   { short_term_fuel_trim_formula,  "S.T. Fuel Trim 1:",        "", "06",      1,    2 },   
   { fuel_eff_formula,              "Fuel Efficiency:",         "", "0D",      1,    2 },
   #endif
...
//Have to make some dialogs longer to fit the wide screen
#ifdef EEEPC_BUILD0
DIALOG sensor_dialog[] =
{
   ...
   { d_shadow_box_proc,      40,  20,  720, 56, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
...
//Sensor 1
   { d_box_proc,             40,  87,  720, 48, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
   { toggle_proc,            45,  91,  45,  40, C_BLACK, C_WHITE,       0,    D_EXIT, 0,   0,   NULL,               NULL, NULL },
   { sensor_proc,            95,  100, 664, 34, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
//Sensor 2
   { d_box_proc,             40,  142, 720, 48, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
   { toggle_proc,            45,  146, 45,  40, C_BLACK, C_WHITE,       0,    D_EXIT, 1,   0,   NULL,               NULL, NULL },
   { sensor_proc,            95,  155, 664, 34, C_BLACK, C_LIGHT_GRAY,  0,    0,      1,   0,   NULL,               NULL, NULL },
//Sensor 3
   { d_box_proc,             40,  197, 720, 48, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
   { toggle_proc,            45,  201, 45,  40, C_BLACK, C_WHITE,       0,    D_EXIT, 2,   0,   NULL,               NULL, NULL },
   { sensor_proc,            95,  210, 664, 34, C_BLACK, C_LIGHT_GRAY,  0,    0,      2,   0,   NULL,               NULL, NULL },
//Sensor 4
   { d_box_proc,             40,  252, 720, 48, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
   { toggle_proc,            45,  256, 45,  40, C_BLACK, C_WHITE,       0,    D_EXIT, 3,   0,   NULL,               NULL, NULL },
   { sensor_proc,            95,  265, 664, 34, C_BLACK, C_LIGHT_GRAY,  0,    0,      3,   0,   NULL,               NULL, NULL },
//Sensor 5
   { d_box_proc,             40,  307, 720, 48, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
   { toggle_proc,            45,  311, 45,  40, C_BLACK, C_WHITE,       0,    D_EXIT, 4,   0,   NULL,               NULL, NULL },
   { sensor_proc,            95,  320, 664, 34, C_BLACK, C_LIGHT_GRAY,  0,    0,      4,   0,   NULL,               NULL, NULL },
//Sensor 6
   { d_box_proc,             40,  362, 720, 48, C_BLACK, C_LIGHT_GRAY,  0,    0,      0,   0,   NULL,               NULL, NULL },
   { toggle_proc,            45,  366, 45,  40, C_BLACK, C_WHITE,       0,    D_EXIT, 5,   0,   NULL,               NULL, NULL },
   { sensor_proc,            95,  375, 664, 34, C_BLACK, C_LIGHT_GRAY,  0,    0,      5,   0,   NULL,               NULL, NULL },
...
#ifdef EEEPC_BUILD1
      fltFuelDensity = get_config_int("general", "fuel_density", TRUE);
      if(fltFuelDensity == 0){
          set_config_float("general", "fuel_density", 711.15);
          fltFuelDensity = 711.15;
      }
#endif
   }
}
...
int page_number_proc(int msg, DIALOG *d, int c)
{
   switch (msg)
   {
      ...
      case MSG_UPDATE:
         ...
         //sprintf(d->dp, "%i of %i", current_page + 1, 12);
         d->flags |= D_DIRTY;
         break;
...
#ifdef EEEPC_BUILD0
    #define SENSOR_LABEL_MARGIN   210
    #define SENSOR_VALUE_INDENT   120
#else
...
int sensor_proc(int msg, DIALOG *d, int c)
{
   ...
   switch (msg)
   {
      ...
      case MSG_DRAW:
         ...
         {
#ifdef EEEPC_BUILD0
            textout_ex(screen, datafile[ARIAL18_FONT].dat, sensor->label, 
                d->x + SENSOR_LABEL_MARGIN - gui_strlen(sensor->label), d->y,
                d->fg, d->bg);
            //if(strcmp(sensor->screen_buf, "N/A") != 0)
                textout_ex(screen, datafile[ARIAL18_FONT].dat, sensor->screen_buf,
                    d->x + SENSOR_LABEL_MARGIN + SENSOR_VALUE_INDENT, d->y,
                    ((d->flags & D_DISABLED) ? gui_mg_color : d->fg), d->bg);
#else           
...
{
#ifdef EEEPC_BUILD0    
//This is way too long for the EEE-PC screen
   if ((data & 0xFF) == 0xFF)  // if the sensor is not used in fuel trim calculation,
      sprintf(buf, "%.3fV", (data >> 8)*0.005);
   else
      sprintf(buf, ((data & 0xFF) == 128) ? "%.3fV 0.0%% st fuel trim" : "%.3fV %+.1f%% st fuel trim", (data >> 8)*0.005, ((float)(data & 0xFF) - 128)*100/128);
#else
   if ((data & 0xFF) == 0xFF)  // if the sensor is not used in fuel trim calculation,
...
#ifdef EEEPC_BUILD1
void fuel_eff_formula(int data, char *buf)
{
    // Density of Gasoline = 702.5 kg/m^3 = 702.5 g / L
    // Air / Fuel ratio under closed loop is close to 14.7
    // MAF / Fuel = 14.7
    // Cosuming MAF / 14.7 gram of fuel per second
    // Which is MAF / 14.7 / Fuel density (702.5) L per second
    // Which is MAF / 14.7 / Fuel density (702.5) x 3600 L per hour
    // Vehicle speed is measured in Km per hour
    // Fuel Efficiency (L / 100Km) = (MAF / 14.7 / Fuel density x 3600) / Vehicle Speed * 100
   
    float fltEff = 0;
            
    if (fltVSS <= 0)
        sprintf(buf, "Vehicle is not moving");
    else  
    {
        //Adjust it with short term fuel trim
        fltEff = (fltMAF / 14.7 / fltFuelDensity * 3600) * (1 + fltTrim * 0.01) / fltVSS * 100;
           
        if (system_of_measurements == METRIC)
            sprintf(buf, "%.2f L / 100Km", fltEff);
        else
            sprintf(buf, "%.2f MPG", 235.2/fltEff);              
    }
} 
#endif
